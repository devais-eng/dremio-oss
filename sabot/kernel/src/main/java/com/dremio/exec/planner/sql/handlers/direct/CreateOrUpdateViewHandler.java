...
public class CreateOrUpdateViewHandler extends SimpleDirectHandler {

  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(CreateOrUpdateViewHandler.class);

  private final SqlHandlerConfig config;
  private final Catalog catalog;
  private BatchSchema viewSchema;

  public CreateOrUpdateViewHandler(SqlHandlerConfig config) {
    this.config = config;
  ...
  public List<SimpleCommandResult> toResult(String sql, SqlNode sqlNode) throws Exception {
    ...
    } catch (CatalogEntityNotFoundException
        | CatalogEntityAlreadyExistsException
        | CatalogUnsupportedOperationException e) {
  ...
  public static CreateOrUpdateViewHandler create(SqlHandlerConfig config) throws SqlParseException {
    try {
      final Class<?> cl =
          Class.forName("com.dremio.exec.planner.sql.handlers.EnterpriseCreateOrUpdateViewHandler");
      final Constructor<?> ctor = cl.getConstructor(SqlHandlerConfig.class);
      return (CreateOrUpdateViewHandler) ctor.newInstance(config);
    } catch (ClassNotFoundException e) {
      return new CreateOrUpdateViewHandler(config);
