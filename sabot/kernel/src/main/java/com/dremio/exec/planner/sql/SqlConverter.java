...
public class SqlConverter {
  ...
  public SqlConverter(
    ...
    this.physicalPlanReader = context.getSabotQueryContext().getPlanReader();
    ...
    this.materializations =
        new MaterializationList(
            ...
            config);
    this.plannerEventBus = new PlannerEventBusImpl();
    this.substitutions =
        buildAccelerationAwareSubstitutionProvider(
            config, context, factory, materializations, observer, settings, plannerEventBus);
    this.planner = DremioVolcanoPlanner.of(this);
    this.cluster =
        RelOptCluster.create(planner, new DremioRexBuilder(JavaTypeFactoryImpl.INSTANCE));
    this.cluster.setMetadataQuerySupplier(relMetadataQuerySupplier);
  ...
  public PhysicalPlanReader getPhysicalPlanReader() {
    ...
    AccelerationAwareSubstitutionProvider substitutions =
        AccelerationAwareSubstitutionProvider.of(
            factory.getSubstitutionProvider(
                sabotConfig, materializations, plannerSettings.getOptions(), plannerEventBus));
